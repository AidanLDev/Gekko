...
strat.log = function(candle) {
  ...
  log.debug(
    '\t',
    'Band gap: ',
    bb.upper.toFixed(digits) - bb.lower.toFixed(digits)
  );
...
strat.check = function(candle) {
  ...
  if (
    price <= bb.lower &&
    rsiVal <= this.settings.rsi.low &&
    macddiff > this.settings.macd.up
  ) {
    // new trend detected
    ...
    if (
      this.trend.duration >= this.settings.rsi.persistence &&
      this.trend.duration >= this.settings.macd.persistence
    ) {
  ...
  if (
    price > bb.middle &&
    rsiVal >= this.settings.rsi.high &&
    macddiff < this.settings.macd.down
  ) {
    // new trend detected
